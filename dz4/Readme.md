...
Было добавлено 2 файла: tests.py и testComands.py. Оба производят проверки, просто первый более подробный, а второй для быстрых проверок. Первый запускается также, как и основная программа (вызывается ассемблер, после интерпретатор). А второй запускается простой командой: python -m pytest testComands.py. Результаты для первого кода лежат в папках, результаты запуска второго файла:

![{BC14660F-C696-4EC0-9265-DF9951F3161D}](https://github.com/user-attachments/assets/46cc9101-fb0f-42e2-837e-2a14e5e0669f)
